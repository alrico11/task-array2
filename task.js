...
newData.forEach(d => {
    console.log('idTransaction', d.idTransaction);
    console.log('storeLocation', d.storeLocation);
    console.log('userEmail', d.userEmail);
    console.log('purchaseMethod', d.purchaseMethod);
...
const locations = [];
data.forEach(d => {
    if (!locations.includes(d.storeLocation)) {
        locations.push(d.storeLocation);
    }
});

const storeloc = data.map(function(d){
    return {
            location: d.storeLocation,
            transaction: {
                idTransaction: d.id,
                purchaseMethod: d.purchaseMethod,
                items: d.items.map((item) => {
...
storeLocations = [
    storeloc.filter((d) => {
        return d.location == 'Denver';
    }),
    storeloc.filter((d) => {
        return d.location == 'Seattle';
    }),
    storeloc.filter((d) => {
        return d.location == 'London';
    }),
    storeloc.filter((d) => {
        return d.location == 'Austin';
    }),
    storeloc.filter((d) => {
        return d.location == 'New York';
    }),
    storeloc.filter((d) => {
        return d.location == 'San Diego';
    }),
];
...
console.log("==============SOAL 3=================================")
// const revenue = newData.map(function(d){
//     return "$"+ (d.items[0].price * d.items[0].qty) 
// })
// console.log(revenue)

const getTotalRev = storeloc.map(function (d) {
    let keuntungan = 0;
    storeloc.forEach(e => {
        keuntungan = keuntungan + d.transaction.items[0].price * d.transaction.items[0].qty
    });
    d.keuntungan = "$" + keuntungan
    return d
})

console.log(getTotalRev)
// console.log("==============SOAL 4=================================")
const quantity = storeloc.map(function (d) {
    d.quantities = d.transaction.items[0].qty
    return d
})
...
// //         .filter((d) => {
// //             return d.storeLocation === location;
// //         })
// //         .map((d) => {
// //             return d.customer.satisfaction.$numberInt;
// //         });
// //     const ratingAverage =
// //         satisfactions.reduce((rating, newData) => {
// //             return rating + parseInt(newData);
// //         }, 0) / satisfactions.length;
// //     ratings[location] = `${ratingAverage} / 5`;
// // }
// // console.log(ratings);
...
//         data
//             .filter(function (d) {
//                 return d.storeLocation == loc
//             })
//             .map(function (d) {
//                 return d.customer.satisfaction.$numberInt
//             })
//     const ratingAverage = satisfactions.reduce(function (rating, value) {
//         return rating + parseInt(value)
//     }, 0) / satisfactions.length
    
//     ratings[loc] = ` ${ratingAverage}`
//     // ratings.location = lo
// });


const ratingMap = data.map(function(d){
    // const ratingAverage = satisfactions.reduce(function (rating, value) {
    //             return rating + parseInt(value)
    //         }, 0) / satisfactions.length
    return {
            location: d.storeLocation,
            revenue: d.customer.satisfaction.$numberInt
            }
})
...
console.log("==============SOAL 6=================================")
// const Locations = data.reduce((location, newData) => {
//     if (!location.includes(newData.storeLocation)) {
//         location.push(newData.storeLocation);
//     }
//     return location;
// }, []);
...
//         .map((d) => {
//             let total = 0;
//             for (const dataBarang of d.items) {
//                 total =
//                     total +
//                     dataBarang.price.$numberDecimal *
//                         dataBarang.quantity.$numberInt;
//             }
//             return total;
//         }).reduce((total, newTotal) => {
//             return total + newTotal
//         });
//         console.log(rev)
//         revenues[location] = rev;
// }
...
//             .map(function (d) {
//                 let total = 0
//                 d.items.forEach(value => {
//                     total = total + (value.price.$numberDecimal * value.quantity.$numberInt)
//                 });
//                 return total
//             }).reduce(function (total, newtotal) {
//                 return total + newtotal
//             })
//     revenues[loc] = `USD ${rev}`
// });
// console.log(revenues)

const revenueMap = storeloc.map(function(d){
    // const ratingAverage = satisfactions.reduce(function (rating, value) {
    //             return rating + parseInt(value)
    //         }, 0) / satisfactions.length
    let keuntungan = 0;
    return {
            location: d.location,
            revenue: `USD ${d.transaction.items[0].price * d.transaction.items[0].qty}`
            }
})
...
arrOmzet = []
const omzetMin = newData.find(function (d) {
    const maxRev = newData.map(function (d) {
        let totalRev = 0;
        newData.forEach(e => {
            totalRev = totalRev + d.items[0].price * d.items[0].qty
        });
        if (!arrOmzet.includes(totalRev)) {
            arrOmzet.push(totalRev);
        }
    })
    let totalRev = 0;
    newData.forEach(e => {
    ...
    const sortedOmzet = arrOmzet.sort(function (a, b) {
        return a - b
    })
})
// const omzetMax = newData.find(function (d) {
//     let totalRev = 0;
//     newData.forEach(e => {
//         totalRev = totalRev + d.items[0].price * d.items[0].qty
//     });
...
//         return a - b
//     })
//     return d.totalRev == '$' + sortedOmzet[sortedOmzet.length - 1]
// })
const totalOmzet = arrOmzet.reduce(function (a, b) {
    return a + b
}, 0)
console.log(`OMZET TERKECIL : USD ${arrOmzet[0]}`)
console.log(`OMZET TERBESAR : USD ${arrOmzet[arrOmzet.length-1]}`)
console.log(`TOTAL OMZET    : USD ${totalOmzet}`)